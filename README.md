...
## 18 个基础工具（15 + 3）+ 可选 You / them / 18 Core Tools (15 + 3) + Optional You / them

18 个基础工具全在一个 MCP 连接器 `/mcp` 上，只配这一个即可。
...
| `notify` | 给晚晚手机发送一条 ntfy 推送。 |
| `check_phone` | 从 Supabase 查询最近的手机使用记录。 |
...
可选的 `You` 与 `them` 不属于这 18 个基础工具。开启时返回的是**模型自己写下的正文**——
